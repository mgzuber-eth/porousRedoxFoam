...
License
    ...
    //redox changes based off 
        opt/src/thermophysicalModels/chemistryModel/chemistryModel/standardChemistryModel/standardChemistryModel.C
        opt/src/thermophysicalModels/chemistryModel/specie/reaction/Reactions/Reaction/Reaction.C
	//for debugging use Info << "Value: " << value << endl;
...
:
    ...
    deltaField_ //redox-delta
    (
      mesh_.lookupObject<volScalarField>("deltaField")
    ),
	ppseudo_ //redox-pseudoKeq
	(
...
) const
{
    ...
    forAll(reactions_, i)
    {
        ...
        scalar omegai = omega //calls omega scalar
        (
            R, c, T, ppseudo_[cellI], nonstoichiometry, pf, cf, lRef, pr, cr, rRef //redox-delta-pseudoKeq
        );  
...
) const
{
    ...
    scalar kf = R.kf(T, ppseudo_[cellI], nonstoichiometry, c1); //redox-delta-pseudoKeq
    //redox-OrderOfReaction
    const label sL = R.slhs().size();
    if (R.glhs().size() > 0)
    {
        forAll(R.glhs(),i)
        {
            if (rhoG_[cellI]*gasPhaseGases_[R.glhs()[i]].internalField()[cellI] > 0.00001)
            {
                kf *= pow(rhoG_[cellI]*gasPhaseGases_[R.glhs()[i]].internalField()[cellI],R.nReact()[sL+i]);
            }
            else
            {
                kf *= pow(0.00001,R.nReact()[sL+i]);    
            }
        }
    }

    scalar kr = R.kr(kf, T, ppseudo_[cellI], nonstoichiometry, c1); //redox-delta-pseudoKeq
    //redox-OrderOfReaction
    scalarList nStoichiometric(R.nReact().size()); //redox-OrderOfReaction
    label position = 0;
    for (label i = 0; i < R.slhsSto().size(); ++i) { nStoichiometric[position++] = 0; }
    for (const auto& value : R.glhsSto()) { nStoichiometric[position++] = value; }
    for (label i = 0; i < R.srhsSto().size(); ++i) { nStoichiometric[position++] = 0; }
    for (const auto& value : R.grhsSto()) { nStoichiometric[position++] = value; }
    
    
    const label Nl = R.slhs().size();
    const label Nlg = R.glhs().size(); //redox
    if (Nl > 0)
    {
        if ( R.glhs().size() > 0 )
        {
            ...
            {
                label si = R.slhs()[s];
                kf *= pow(Ys_[si][cellI],nStoichiometric[s]);
            }
            forAll(R.glhs(),i)
            {
                kf *= pow(gasPhaseGases_[R.glhs()[i]].internalField()[cellI],nStoichiometric[Nl+i]);
            }
        }
        else
        {
    ...
    else
    {
        forAll(R.glhs(),i)
        {
            kf *= pow(gasPhaseGases_[R.glhs()[i]].internalField()[cellI],nStoichiometric[i]);
        }
        kf *= rhoG_[cellI];
    }
    
    //redox
    const label Nr = R.srhs().size();
    if (Nr>0)
    {
        if ( R.grhs().size() > 0 )
        {
            ...
            {
            label si = R.srhs()[s];
                kr *= pow(Ys_[si][cellI],nStoichiometric[s+Nl+Nlg]); //redox
            }
            forAll(R.grhs(),i)
            {
                kr *= pow(gasPhaseGases_[R.grhs()[i]].internalField()[cellI],nStoichiometric[Nr+i+Nl+Nlg]); //redox
            }
        }
        else
        {
            ...
            {
                label si = R.srhs()[s];
                kr *= pow(Ys_[si][cellI],nStoichiometric[s]);
			}
        }
        kr *= this->solidThermo().rho()[cellI];
    }
    else
    {
        forAll(R.grhs(),i)
        {
            kr *= pow(gasPhaseGases_[R.grhs()[i]].internalField()[cellI],nStoichiometric[i]);
        }
        kr *= rhoG_[cellI];
    }
    ...
    //Diffusion Limited case added for kr for now, but commented out 
    /*if (diffusionLimitedReactions_ and (R.grhs().size() > 0 ) and (kr > VSMALL))
    {
        scalar avKr = 1./kr;
        forAll(R.grhs(),i)
        {
            scalar addAvKr = (ST_[cellI]*gasPhaseGases_[R.grhs()[i]].internalField()[cellI]*rhoG_[cellI]);
...
) const
{
    ...
    scalar nonstoichiometry = deltaField_[celli]; //redox-delta
    

    scalarField omegaPreq(omega(c,T,ppseudo_[celli],nonstoichiometry,rR) * (1. - porosityF_[celli])); //redox-delta-pseudoKeq, calls omega field
    forAll(dcdt,i)
    {
        dcdt[i] = omegaPreq[i];
    }
    ...
    for (label i=0; i<nSolids_; i++)
    {
            ...
            newhi -= dYidt*solidThermo_[i].hf(T,nonstoichiometry);//redox-Hfd
    }
    }
    else
    {
...
) const
{
    ...
    scalarField omegaPreq(omega(c2,T,ppseudo_[cellI],nonstoichiometry,rR)*(1.-porosityF_[cellI])); //redox-delta-pseudoKeq, calls omega field
    ...
    for (label ri=0; ri<reactions_.size(); ri++)
    {
        ...
        const label Nsr = R.srhs().size(); //redox
        const label Ngr = R.grhs().size(); //redox
        ...
        scalar kf0 = R.kf(T, ppseudo_[cellI], nonstoichiometry, c2); //redox-delta-pseudoKeq
        //- OrderOfReaction
        const label sL = R.slhs().size();
        if (R.glhs().size() > 0)
        {
            forAll(R.glhs(),i)
            {
                kf0 *= pow(rhoG_[cellI]*gasPhaseGases_[R.glhs()[i]].internalField()[cellI],-1.*R.nReact()[sL+i]);
            }
        }

        scalar kr0 = R.kr(kf0, T, ppseudo_[cellI], nonstoichiometry, c2); //redox-delta-pseudoKeq, calls kr 
        ...
        if (Nsr > 0)
        {
            kr0 *= this->solidThermo().rho()[cellI];
        }
        else
        {
            kr0 *= rhoG_[cellI];
        }
        kr0 *= (1.-porosityF_[cellI]);
        ...
        for (label rSj=0; rSj < Ns + Ng; rSj++)
        {
            ...
            {
                sj = gasDictionaryBack_[R.glhs()[rSj-Ns]] + nSolids_; //sj gives index of species on the lhs
            }

            scalar kf = kf0;
            scalar kfTot = kf0;

            scalar nStoichiometric; //redox-OrderOfReaction
            ...
            for (label rSi=0; rSi < Ns + Ng; rSi++)
            {
                ...
                {
                    si = R.slhs()[rSi];
                    nStoichiometric = 0;//redox-OrderOfReaction
                }
                else
                {
                    si = gasDictionaryBack_[R.glhs()[rSi-Ns]] + nSolids_;
                    nStoichiometric = R.glhsSto()[rSi-Ns];//redox-OrderOfReaction
                }

                scalar el = nStoichiometric;//redox-OrderOfReaction
        ...
        //redox
        for (label rSj=0; rSj < Nsr + Ngr; rSj++)
        {
            label sj;
            if (rSj < Nsr)
            {
            ...
            {
                sj = gasDictionaryBack_[R.grhs()[rSj-Nsr]] + nSolids_;
            }
            ...
            for (label rSi=0; rSi < Nsr + Ngr; rSi++)
            {
                label si;
                if (rSi < Nsr)
                {
                ...
                {
                    si = gasDictionaryBack_[R.grhs()[rSi-Nsr]] + nSolids_;
                    nStoichiometric = R.grhsSto()[rSi-Nsr];//redox-OrderOfReaction
                }

                scalar er = nStoichiometric;//redox-OrderOfReaction
                if (rSi < Nsr)
                {
                    krTot *= pow(Ys_[si][cellI],er);
                }
                else
                {
                    krTot *= pow(gasPhaseGases_[R.grhs()[rSi-Nsr]].internalField()[cellI],er);
                }
                ...
                if (rSi == rSj) 
                {
                    if (er < 1.0)
                    {
                        if (c2[si]>SMALL)
                        {
                            if (rSi < Nsr)
                            {
                                kr *= er*pow(Ys_[si][cellI] + VSMALL, er - 1.0);
                            }
                            else
                            {
                                kr *= er*pow(gasPhaseGases_[R.grhs()[rSi-Nsr]].internalField()[cellI] + VSMALL, er - 1.0);
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        if (rSi < Nsr)
                        {
                            kr *= er*pow(Ys_[si][cellI], er - 1.0); 
                        }
                        else
                        {
                            kr *= er*pow(gasPhaseGases_[R.grhs()[rSi-Nsr]].internalField()[cellI], er - 1.0);
                        }
                    }
                }
                else
                {
                     if (rSi < Nsr)
                     {
                         kr *= pow(Ys_[si][cellI],er);
                     }
                     else
                     {
                         kr *= pow(gasPhaseGases_[R.grhs()[rSi-Nsr]].internalField()[cellI],er);
                     }
                }
            }
            ...
            for (label rSi=0; rSi < Nsr + Ngr; rSi++)
            {
                ...
                dfdc[si][sj] -= kr * stCoeffs[si]; //redox
            }
            ...
            {
                label gi = R.glhs()[g]; 
                dfdc[gi+nSolids_][sj] -= kr*stCoeffs[gi+nSolids_]; //redox
            }
        }
    ...
    omegaPreq = omega(c2,T - delta ,ppseudo_[cellI],nonstoichiometry,rR)*(1.-porosityF_[cellI]); //redox-delta-pseudoKeq, calls omega field
    ...
    omegaPreq = omega(c2,T + delta,ppseudo_[cellI],nonstoichiometry,rR)*(1.-porosityF_[cellI]); //redox-delta-pseduoKeq, calls omega field
    forAll(dcdT1,i)
    {
        dcdT1[i] = omegaPreq[i];
    }
...
Foam::ODESolidHeterogeneousChemistryModel<SolidThermo, SolidThermoType, GasThermoType>::Sh() const
{
    ...
    if (this->chemistry_)
    {
        ...
        if (solidReactionEnergyFromEnthalpy_)
        {
            ...
            forAll(Sh, cellI)
            {
                scalar TTemp = this->solidThermo().T()[cellI]; //redox-Hfd
                scalar nonstoichiometryTemp = deltaField_[cellI]; //redox-Hfd
                scalar hf = solidThermo_[i].hf(TTemp, nonstoichiometryTemp); //redox-Hfd
    ...
    if (this->chemistry_)
    {
        forAll(rho, celli)
        {
            ...
            if (reactingCells_[celli])
            {
                ...
                scalarField rR(nReaction_, 0.0);
                Info << "is this ever actually used?" << endl;
                scalar nonstoichiometry = deltaField_[celli]; //redox-delta
                const scalarField dcdt = omega(c, Ti, ppseudo_[celli], nonstoichiometry, rR, true); //redox-delta-pseudoKeq, calls omega field
                forAll(RRs_, i)
                {
...
)
{
    ...
    if (reactingCells_[celli])
    {
        ...
        scalarField omegaPreq(omega(initialSpecieConcentration, Ti, ppseudo_[celli], nonstoichiometry, rR) * (1. - porosityF_[celli])); //redox-delta-pseudoKeq, calls omega field
        ...
        calculateSourceTerms(t0, deltaT, deltaTMin, celli, solidRho, Ti, nonstoichiometry, initialSpecieConcentration, omegaPreq); //redox-Hfd
...
(
    ...
    scalar& Ti,
    scalar& nonstoichiometry, //redox-Hfd
    ...
    while (timeLeft > SMALL)
    {
        ...
        if (solidReactionEnergyFromEnthalpy_)
        {
            for (label i=0; i < nSolids_; i++)
            {
                ...
                newhi -= dYi * solidThermo_[i].hf(Ti, nonstoichiometry); //redox-Hfd
