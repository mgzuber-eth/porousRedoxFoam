...
Foam::HGSSolidMixtureThermo<MixtureType>::hs() const
{
    ...
    forAll(T_.internalField(), celli)
    {
        hs[celli] = MixtureType::hs(T_[celli], deltaField_[celli], celli); //redox-Hfd
    }
...
Foam::HGSSolidMixtureThermo<MixtureType>::Hf() const
{
    ...
    forAll(T_.internalField(), celli)
    {
        hf[celli] = MixtureType::hf(T_[celli], deltaField_[celli], celli); //redox-Hfd
    }
...
{
    const scalarField& patchT = T_.boundaryField()[patchI];
    const scalarField& patchNonstoichiometry = deltaField_.boundaryField()[patchI];//redox-Hfd
    const polyPatch& pp = mesh_.boundaryMesh()[patchI];
    const unallocLabelList& cells = pp.faceCells();
    ...
    forAll(patchT, celli)
    {
        hs[celli] = MixtureType::hs(patchT[celli], patchNonstoichiometry[celli], cells[celli]);//redox-Hfd
    }
...
) const
{
    ...
    forAll(patchT, celli)
    {
        Hf[celli] = MixtureType::hf(patchT[celli], patchNonstoichiometry[celli], cells[celli]); //redox-Hfd
    }
