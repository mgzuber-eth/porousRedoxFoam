...
void Foam::multiComponentSolidMixture<ThermoSolidType>::correctMassFractions()
{
    ...
    forAll(Y_, n)
    {
        Y_[n].primitiveFieldRef() /= Yt.internalField();
    }
}
...
(
    scalar T, scalar nonstoichiometry, label celli //redox-Hfd
) const
{
    scalar tmp = 0.0;
    forAll(solidData_, i)
    {
        tmp += solidData_[i].hf(T, nonstoichiometry) * Y_[i][celli];//redox-Hfd
    }
    return tmp;
}
...
) const
{
    ...
    forAll(solidData_, i)
    {
        tmp += solidData_[i].hs(T,nonstoichiometry)*Y_[i][celli];//redox-Hfd
    }
    return tmp;
}
...
) const
{
    ...
    forAll(solidData_, i)
    {
        tmp += solidData_[i].h(T,nonstoichiometry)*Y_[i][celli];//redox-Hfd
    }
    return tmp;
}
