...
) const
{
    forAll (cells, i)
    {
        ...
        {
            dragCoeff = (
                            mu[cells[i]] + f_*rho[cells[i]]*mag(U[cells[i]])*sqrt(sqrt(3.))/sqrt(mag(Df[cells[i]]))//redox-Forchh : original line = mu[cells[i]] + f_*rho[cells[i]]*mag(U[cells[i]])*sqrt(3.)/mag(Df[cells[i]])
                        )*Df[cells[i]];
        }
        else
        {
